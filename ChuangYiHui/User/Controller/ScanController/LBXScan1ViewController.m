...
@interface LBXScan1ViewController ()


@end

@implementation LBXScan1ViewController
...
//    vc.strCodeType = strResult.strBarCodeType;
    NSArray *strArr = [strResult.strScanned componentsSeparatedByString:@"|"];
    if ([strArr[0] isEqualToString:@"user"]){
        UserDetailController *vc = [UserDetailController new];
        UserModel *model = [[UserModel alloc] init];
        model.user_id = strArr[1];
        vc.model = model;
        [self.navigationController pushViewController:vc animated:YES];
    }else if([strArr[0] isEqualToString:@"team"]){
        TeamDetailController *vc = [TeamDetailController new];
        TeamModel *model = [TeamModel new];
        model.team_id = strArr[1];
        vc.teamModel = model;
        [self.navigationController pushViewController:vc animated:YES];
    }
}
