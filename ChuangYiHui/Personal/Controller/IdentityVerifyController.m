...
@interface IdentityVerifyController ()<UITableViewDelegate, UITableViewDataSource, UIActionSheetDelegate, UIImagePickerControllerDelegate, SchoolPickerViewDelegate, UIImagePickerControllerDelegate>
...
@implementation IdentityVerifyController
...
- (void)initView{
    ...
    [_tableView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(6 * RowHeight + 3 * 14);
    }];
...
- (void)enterButtonAction{
    if ([self checkInput]) {
       [[NetRequest sharedInstance] httpRequestWithPost:URL_IDENTITY_VERIFY parameters:_params withToken:NO success:^(id data, NSString *message) {
...
- (BOOL)checkInput{
    ...
    if (_selectIdentity == 0) {
        [_params setObject:@"学生" forKey:@"role"];
        //学生
        ...
        if ([academyCell getOnlyContent].length == 0) {
            [[SVHudManager sharedInstance] showErrorHudWithTitle:@"请输入学院信息" andTime:1.0f];
            return NO;
        }
        [_params setObject:[academyCell getOnlyContent] forKey:@"unit2"];
        ...
        if ([majorCell getOnlyContent].length == 0) {
            [[SVHudManager sharedInstance] showErrorHudWithTitle:@"请输入专业信息" andTime:1.0f];
            return NO;
        }
    
        [_params setObject:[majorCell getOnlyContent] forKey:@"profession"];


        
    }else if(_selectIdentity == 1){
        [_params setObject:@"教师" forKey:@"role"];
        //教师
        indexPath = [NSIndexPath indexPathForRow:0 inSection:1];
        LabelLabelArrowCell *schoolCell = [_tableView cellForRowAtIndexPath:indexPath];
        if ([schoolCell getOnlyContent].length == 0) {
        ...
        [_params setObject:[academyCell getOnlyContent] forKey:@"unit2"];
        [_params setObject:@"教师" forKey:@"profession"];

    }else{
        [_params setObject:@"在职" forKey:@"role"];
        //在职
        indexPath = [NSIndexPath indexPathForRow:0 inSection:1];
        LabelFieldCell *academyCell = [_tableView cellForRowAtIndexPath:indexPath];
        if ([academyCell getOnlyContent].length == 0) {
            [[SVHudManager sharedInstance] showErrorHudWithTitle:@"请输入单位信息" andTime:1.0f];
            return NO;
        }
        [_params setObject:[academyCell getOnlyContent] forKey:@"unit1"];
        [_params setObject:[academyCell getOnlyContent] forKey:@"unit2"];
        indexPath = [NSIndexPath indexPathForRow:1 inSection:1];
        LabelFieldCell *majorCell = [_tableView cellForRowAtIndexPath:indexPath];
        if ([majorCell getOnlyContent].length == 0) {
            [[SVHudManager sharedInstance] showErrorHudWithTitle:@"请输入职业信息" andTime:1.0f];
            return NO;
        }
        [_params setObject:[majorCell getOnlyContent] forKey:@"profession"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0 || (indexPath.section == 2 && indexPath.row == 1) || (indexPath.section == 1 && indexPath.row == 0 && _selectIdentity != 2)) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        //身份
        [ActionSheetStringPicker showPickerWithTitle:@"请选择身份" rows:@[@[@"学生", @"教师", @"在职"]] initialSelection:@[@0] doneBlock:^(ActionSheetStringPicker *picker, NSArray *selectedIndex, NSArray *selectedValue) {
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info{
    ...
    if (EditedImage) {
        ...
        [[NetRequest sharedInstance] upLoad:URL_UPLOAD_OTHER_IMAGE parameters:@{} imageKey:@[@"image"] imageArray:@[EditedImage] success:^(id data, NSString *message) {
