//
//  TaskInternalReleaseController.m
//  ChuangYiHui
//
...
@interface TaskInternalReleaseController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation TaskInternalReleaseController
...
- (void)enterButtonAction{
    if (_enterWay == 1) {
        ...
        if ([self checkInput]) {
            ...
            [[NetRequest sharedInstance] httpRequestWithPost:URL_GET_TEAM_INTERNAL_TASKS(_model.team_id) parameters:params withToken:NO success:^(id data, NSString *message) {
...
- (void)resignTask{
    if ([self checkInput]) {
        ...
        [[NetRequest sharedInstance] httpRequestWithPost:URL_CHANGE_INTERNAL_TASKS(_taskModel.task_id) parameters:params withToken:NO success:^(id data, NSString *message) {
...
- (BOOL)checkInput{
    ...
    if ([[memberCell getOnlyContent] length] == 0) {
        [[SVHudManager sharedInstance] showErrorHudWithTitle:@"请选择执行人" andTime:1.0f];
        return NO;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        //执行人
        if (_memberNameArr.count == 0) {
            [[SVHudManager sharedInstance] showErrorHudWithTitle:@"该团队还没有成员" andTime:1.0f];
        }else{
            if (_enterWay == 0) {
                //创建任务
                [ActionSheetStringPicker showPickerWithTitle:@"选择执行人员" rows:@[_memberNameArr] initialSelection:@[@0] doneBlock:^(ActionSheetStringPicker *picker, NSArray *selectedIndex, NSArray *selectedValue) {
                    ...
                    UserModel *userModel = [_memberArr objectAtIndex:[selectedIndex[0] integerValue]];
                    _executorId = userModel.user_id;
